...
from robots.body_layouts import build_arm_command_mapping
from robots.hand_layouts import build_inspire_primary_joint_mapping, build_inspire_special_joint_mapping
class DDSActionProvider(ActionProvider):
    ...
    def __init__(self,env, args_cli):
        ...
        self._last_inspire_debug_ts = 0.0
        self._last_arm_debug_ts = 0.0
        self._setup_dds()
        self._setup_joint_mapping()

    def _maybe_log_inspire_debug(self, inspire_cmds) -> None:
        now = time.time()
        if now - self._last_inspire_debug_ts < 1.0:
            return
        self._last_inspire_debug_ts = now

        if not inspire_cmds:
            print("[DDSActionProvider][inspire-debug] no hand command received")
            return

        positions = inspire_cmds.get("positions")
        if positions is None:
            print(f"[DDSActionProvider][inspire-debug] missing positions field: keys={list(inspire_cmds.keys())}")
            return

        rounded_positions = [round(float(val), 3) for val in positions[:12]]
        mapping_preview = {
            name: rounded_positions[idx]
            for name, idx in self.inspire_hand_joint_mapping.items()
            if idx < len(rounded_positions)
        }
        print(
            f"[DDSActionProvider][inspire-debug] "
            f"shm_timestamp={inspire_cmds.get('_timestamp')} "
            f"positions={rounded_positions} "
            f"mapped={mapping_preview}"
        )
    
    def _maybe_log_arm_debug(self, raw_positions, full_action) -> None:
        now = time.time()
        if now - self._last_arm_debug_ts < 1.0:
            return
        self._last_arm_debug_ts = now

        arm_cmd_preview = {
            name: round(float(full_action[self.joint_to_index[name]].item()), 3)
            for name in self.arm_joint_mapping.keys()
        }
        current_preview = {}
        joint_pos = getattr(self.env.scene["robot"].data, "joint_pos", None)
        if joint_pos is not None:
            current_preview = {
                name: round(float(joint_pos[0, self.joint_to_index[name]].item()), 3)
                for name in self.arm_joint_mapping.keys()
            }
        src_preview = {
            idx: round(float(raw_positions[idx]), 3)
            for idx in self._arm_source_indices
            if idx < len(raw_positions)
        }
        print(
            f"[DDSActionProvider][arm-debug] src={src_preview} cmd={arm_cmd_preview} current={current_preview}"
        )
    ...
    def _setup_joint_mapping(self):
        ...
        if self.enable_robot == "g129":
            self.arm_joint_mapping = build_arm_command_mapping(self.all_joint_names)
            ...
            self._arm_source_indices = [self.arm_joint_mapping[name] for name in self.arm_joint_mapping.keys()]
        ...
        if self.enable_inspire:
            self.inspire_hand_joint_mapping = build_inspire_primary_joint_mapping(self.all_joint_names)
            self.special_joint_mapping = build_inspire_special_joint_mapping(self.all_joint_names)
    ...
    def get_action(self, env) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            if self.enable_robot == "g129" and self.robot_dds:
                ...
                if cmd_data and 'motor_cmd' in cmd_data:
                    ...
                    if len(positions) >= 29:
                        ...
                        self._maybe_log_arm_debug(positions, full_action)
            ...
            if self.gripper_dds:
                ...
                self._maybe_log_inspire_debug(inspire_cmds)
