...
from robots.hand_layouts import uses_x200_hand_layout
import os

def _configure_inspire_hand_profile(env):
    try:
        joint_names = env.scene["robot"].data.joint_names
        profile = "x200" if uses_x200_hand_layout(joint_names) else "legacy"
    except Exception:
        profile = "x200"
    os.environ["UNITREE_INSPIRE_HAND_PROFILE"] = profile
    print(f"[dds_create] UNITREE_INSPIRE_HAND_PROFILE={profile}")
    return profile
...
def create_dds_objects(args_cli,env):
    ...
    if args_cli.enable_dex3_dds:
        ...
        _configure_inspire_hand_profile(env)
