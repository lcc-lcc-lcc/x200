...
import numpy as np
from robots.hand_layouts import get_inspire_dds_joint_limits
...
class InspireDDS(DDSObject):
    ...
    def __init__(self,node_name:str="inspire"):
        ...
        self._profile = os.environ.get("UNITREE_INSPIRE_HAND_PROFILE", "x200")
        self._joint_limits = get_inspire_dds_joint_limits(self._profile)
        print(f"[{self.node_name}] Inspire hand profile: {self._profile}")
    ...
    def dds_publisher(self) -> Any:
        ...
        try:
            ...
            if all(key in data for key in ["positions", "velocities", "torques"]):
                ...
                for i in range(min(len(self._joint_limits), len(positions), len(self.inspire_hand_state.states))):
                    min_val, max_val = self._joint_limits[i]
                    inspire_q_value = self.normalize(float(positions[i]), min_val, max_val)
    ...
    def dds_subscriber(self, msg: MotorCmds_,datatype:str=None) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(min(len(self._joint_limits), len(msg.cmds))):
                min_val, max_val = self._joint_limits[i]
                joint_angle = self.denormalize(float(msg.cmds[i].q), min_val, max_val)
