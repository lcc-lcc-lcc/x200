...
class CameraPresets:
    ...
    @classmethod
    def x200_front_camera(cls) -> CameraCfg:
        """Front camera configuration for the X200 model.

        The camera is mounted directly on ``d435_link`` so the translation comes from the URDF/USD
        ``d435_joint``. We still keep the standard IsaacLab camera-frame correction quaternion here,
        which converts the parent link's robot/world-style axes into the camera optical frame.
        """
        return CameraBaseCfg.get_camera_config(
    ...
    @classmethod
    def left_x200_wrist_camera(cls) -> CameraCfg:
        """left wrist camera configuration for the X200 hand model"""
        return CameraBaseCfg.get_camera_config(
            prim_path="/World/envs/env_.*/Robot/left_wrist_roll_link/left_wrist_camera",
            height=480,
            width=640,
            update_period=0.02,
            data_types=["rgb"],
            ...
            horizontal_aperture=20.0,
            clipping_range=(0.1, 1.0e5),
            pos_offset=(0.16191028, 0.045, 0.135),
            rot_offset=(-0.34202, 0.93969, 0.0, 0.0),
        )
    @classmethod
    def right_x200_wrist_camera(cls) -> CameraCfg:
        """right wrist camera configuration for the X200 hand model"""
        return CameraBaseCfg.get_camera_config(
            prim_path="/World/envs/env_.*/Robot/right_wrist_roll_link/right_wrist_camera",
            height=480,
            width=640,
            update_period=0.02,
            data_types=["rgb"],
            ...
            horizontal_aperture=20.0,
            clipping_range=(0.1, 1.0e5),
            pos_offset=(0.11108972, 0.045, 0.135),
            rot_offset=(-0.34202, 0.93969, 0.0, 0.0),
        )
