...
from robots.body_layouts import X200_ARM_INIT_POS, X200_TORSO_INIT_POS
from robots.hand_layouts import X200_HAND_INIT_POS
from robots.unitree import G129_CFG_WITH_DEX1_BASE_FIX,G129_CFG_WITH_DEX3_BASE_FIX,G129_CFG_WITH_INSPIRE_HAND,G129_CFG_WITH_X200_HAND,G129_CFG_WITH_DEX1_WHOLEBODY,G129_CFG_WITH_DEX3_WHOLEBODY,G129_CFG_WITH_INSPIRE_WHOLEBODY,H12_CFG_WITH_INSPIRE_HAND
...
class RobotJointTemplates:
    ...
    @classmethod
    def get_arm_joints(cls, arm_type: Literal["g1", "x200"] = "g1") -> Dict[str, float]:
        """get the default position of the arm joints"""
        if arm_type == "x200":
            return dict(X200_ARM_INIT_POS)
        return {
            # left arm joint
    ...
    @classmethod
    def get_hand_joints(cls, hand_type: Literal["gripper", "dex3", "inspire", "x200"] = "gripper") -> Dict[str, float]:
        ...
        if hand_type == "gripper":
            ...
            return dict(X200_HAND_INIT_POS)
        else:
            raise ValueError(f"Unsupported hand type: {hand_type}. Supported: 'gripper', 'dex3', 'inspire', 'x200'")
...
class RobotBaseCfg:
    ...
    def get_base_config(
        ...
        if update_default_joint_pos:
            ...
            # add the waist or torso joints
            if robot_type == "g129dof":
                default_joint_pos.update(RobotJointTemplates.get_waist_joints(include_waist))
            elif robot_type == "x200":
                default_joint_pos.update(X200_TORSO_INIT_POS)
            
            # add the arm joints
            arm_type = "x200" if robot_type == "x200" else "g1"
            default_joint_pos.update(RobotJointTemplates.get_arm_joints(arm_type))
...
class G1RobotPresets:
    ...
    @classmethod
    def g1_29dof_x200_base_fix(cls,init_pos: Tuple[float, float, float] = (-0.15, 0.0, 0.76),
        init_rot: Tuple[float, float, float, float] = (0.7071, 0, 0, 0.7071)) -> ArticulationCfg:
        """pick-place task configuration - x200 hand"""
        return RobotBaseCfg.get_base_config(
            init_pos=init_pos,
            init_rot=init_rot,
            include_waist=False,
            hand_type="x200",
            base_config=G129_CFG_WITH_X200_HAND,
            robot_type="x200"
        )
