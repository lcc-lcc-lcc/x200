...
# left_q_target/right_q_target use the Inspire motor semantic order:
# [pinky, ring, middle, index, thumb bend, thumb rotation].
# Retargeting produces x200 joint angles in radians, while the current
# Inspire control interfaces expect normalized command values in [0, 1].
LEGACY_INSPIRE_MOTOR_LIMITS = (
    (0.0, 1.7),
    (0.0, 1.7),
    (0.0, 1.7),
    (0.0, 1.7),
...
HAND_PROFILE_TO_LIMITS = {
    "legacy": LEGACY_INSPIRE_MOTOR_LIMITS,
    "x200": X200_MOTOR_LIMITS,
}


def normalize_q_targets(q_target, motor_limits):
    normalized = np.array(q_target, dtype=np.float32, copy=True)
    for idx, (min_val, max_val) in enumerate(motor_limits):
        normalized[idx] = np.clip((max_val - normalized[idx]) / (max_val - min_val), 0.0, 1.0)
    return normalized


def smooth_and_deadband(prev_q_target, current_q_target, smoothing=0.35, deadband=0.02):
    current_q_target = np.array(current_q_target, dtype=np.float32, copy=True)
    if prev_q_target is None:
        return np.clip(current_q_target, 0.0, 1.0)

    filtered_q_target = smoothing * prev_q_target + (1.0 - smoothing) * current_q_target
    filtered_q_target[np.abs(filtered_q_target - prev_q_target) < deadband] = prev_q_target[
        np.abs(filtered_q_target - prev_q_target) < deadband
    ]
    return np.clip(filtered_q_target, 0.0, 1.0)


def _should_log_debug(owner, attr_name, interval_s=1.0):
    now = time.time()
    last = getattr(owner, attr_name, 0.0)
...
def maybe_log_hand_debug_input(owner, tag, left_hand_data, right_hand_data, hand_input_valid):
    if not _should_log_debug(owner, "_last_hand_input_log_ts"):
        return
    left_probe = np.round(left_hand_data[4], 4).tolist()
    right_probe = np.round(right_hand_data[4], 4).tolist()
    logger_mp.info(
        f"[{tag}] hand_input_valid={hand_input_valid} "
        f"left_all_zero={bool(np.all(left_hand_data == 0.0))} "
        f"right_all_zero={bool(np.all(right_hand_data == 0.0))} "
        f"left_probe={left_probe} right_probe={right_probe}"
    )


def maybe_log_x200_targets(owner, tag, raw_left, raw_right, norm_left, norm_right):
    if not _should_log_debug(owner, "_last_x200_target_log_ts"):
        return
    logger_mp.info(
        f"[{tag}] raw_left={np.round(raw_left, 3).tolist()} "
        f"raw_right={np.round(raw_right, 3).tolist()} "
        f"norm_left={np.round(norm_left, 3).tolist()} "
        f"norm_right={np.round(norm_right, 3).tolist()}"
    )

class Inspire_Controller_DFX:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
                       dual_hand_action_array = None, fps = 100.0, Unit_Test = False, simulation_mode = False, hand_profile = "x200"):
        logger_mp.info("Initialize Inspire_Controller_DFX...")
        self.fps = fps
        self.Unit_Test = Unit_Test
        self.simulation_mode = simulation_mode
        self.hand_profile = "legacy" if hand_profile == "legacy" else "x200"
        self.motor_limits = HAND_PROFILE_TO_LIMITS[self.hand_profile]
        self.command_smoothing = 0.35
        self.command_deadband = 0.02
        if self.hand_profile == "legacy":
            hand_type = HandType.INSPIRE_HAND if not self.Unit_Test else HandType.INSPIRE_HAND_Unit_Test
        else:
            hand_type = HandType.X200_HAND if not self.Unit_Test else HandType.X200_HAND_Unit_Test
    ...
    def control_process(self, left_hand_array, right_hand_array, left_hand_state_array, right_hand_state_array,
        ...
        left_q_target  = np.full(Inspire_Num_Motors, 1.0, dtype=np.float32)
        right_q_target = np.full(Inspire_Num_Motors, 1.0, dtype=np.float32)
        ...
        try:
            while self.running:
                ...
                hand_input_valid = not np.all(right_hand_data == 0.0) and not np.all(left_hand_data[4] == np.array([-1.13, 0.3, 0.15]))
                maybe_log_hand_debug_input(self, "Inspire_Controller_DFX", left_hand_data, right_hand_data, hand_input_valid)

                if hand_input_valid:  # if hand data has been initialized.
                    ref_left_value = left_hand_data[self.hand_retargeting.left_indices[1,:]] - left_hand_data[self.hand_retargeting.left_indices[0,:]]
                    ref_right_value = right_hand_data[self.hand_retargeting.right_indices[1,:]] - right_hand_data[self.hand_retargeting.right_indices[0,:]]
                    left_q_target_raw = self.hand_retargeting.left_retargeting.retarget(ref_left_value)[self.hand_retargeting.left_dex_retargeting_to_hardware]
                    right_q_target_raw = self.hand_retargeting.right_retargeting.retarget(ref_right_value)[self.hand_retargeting.right_dex_retargeting_to_hardware]
                    left_q_target_norm = normalize_q_targets(left_q_target_raw, self.motor_limits)
                    right_q_target_norm = normalize_q_targets(right_q_target_raw, self.motor_limits)
                    left_q_target = smooth_and_deadband(left_q_target, left_q_target_norm, smoothing=self.command_smoothing, deadband=self.command_deadband)
                    right_q_target = smooth_and_deadband(right_q_target, right_q_target_norm, smoothing=self.command_smoothing, deadband=self.command_deadband)
                    maybe_log_x200_targets(self, "Inspire_Controller_DFX", left_q_target_raw, right_q_target_raw, left_q_target, right_q_target)
    ...
    def control_process(self, left_hand_array, right_hand_array, left_hand_state_array, right_hand_state_array,
        ...
        try:
            while self.running:
                ...
                hand_input_valid = not np.all(right_hand_data == 0.0) and not np.all(left_hand_data[4] == np.array([-1.13, 0.3, 0.15]))
                maybe_log_hand_debug_input(self, "Inspire_Controller_FTP", left_hand_data, right_hand_data, hand_input_valid)
                ...
                if hand_input_valid:  # if hand data has been initialized.
                    ...
                    left_q_target = smooth_and_deadband(left_q_target, left_q_target_norm, smoothing=self.command_smoothing, deadband=self.command_deadband)
                    right_q_target = smooth_and_deadband(right_q_target, right_q_target_norm, smoothing=self.command_smoothing, deadband=self.command_deadband)
                    maybe_log_x200_targets(self, "Inspire_Controller_FTP", left_q_target_raw, right_q_target_raw, left_q_target, right_q_target)
